...
begin
  ...
  if Trim(ItensOrdemSERIE.AsString) = '' then
    raise Exception.Create('Informe a Série.');
  ...
  begin
    ...
    begin
      Qsql.Close;
      raise Exception.Create('Lote para o Agente Extintor 15 inválido.');
    end;
    Qsql.Close;
...
begin
  ...
  if OrdemDATAORDEM.AsString = '' then
    raise Exception.Create('Informe a Data de Emissão.');
...
begin
  if CompraNUMORDEM.AsInteger = 0 then
    raise Exception.Create('Informe o Número da Compra.');
