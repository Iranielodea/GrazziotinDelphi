...
procedure TfManutencao.FormActivate(Sender: TObject);
begin
   if sheetLista.Showing then
      Grade.SetFocus;
end;

procedure TfManutencao.butCancelarClick(Sender: TObject);
begin
...
procedure TfManutencao.butOkClick(Sender: TObject);
var tm: string[1];
begin
   dm.IniTrans;
   tm:='A';
...
procedure TfManutencao.GradeDblClick(Sender: TObject);
begin
...
procedure TfManutencao.GradeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfManutencao.FormKeyPress(Sender: TObject; var Key: Char);
begin
   if key = #13 then
   begin
      key:=#0;
...
procedure TfManutencao.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfManutencao.butIncClick(Sender: TObject);
begin
   sheetLista.TabVisible:=false;
   sheetCad.TabVisible:=true;
   dsCad.DataSet.Append;
...
procedure TfManutencao.butExcClick(Sender: TObject);
begin
   if dsCad.DataSet.IsEmpty then
      exit;
   if dm.Integridade('TIORDEM','CODNIVEL',dm.ManutencaoCODMANUTENCAO.Text) = false then
      exit;
...
procedure TfManutencao.ButFecharClick(Sender: TObject);
begin
   close;
end;

procedure TfManutencao.butPesqClick(Sender: TObject);
begin
...
procedure TfManutencao.GradeEnter(Sender: TObject);
begin
   fManutencao.KeyPreview:=false;
end;

procedure TfManutencao.GradeExit(Sender: TObject);
begin
   fManutencao.KeyPreview:=true;
end;

procedure TfManutencao.butAltClick(Sender: TObject);
begin
   if dsCad.DataSet.IsEmpty then
      exit;
   dsCad.DataSet.Edit;
...
procedure TfManutencao.butConfClick(Sender: TObject);
begin
   vgcodigo:=dsCad.DataSet.Fields[0].Text;
   close;
end;

procedure TfManutencao.CodigoExit(Sender: TObject);
begin
   if Codigo.Modified then
   begin
      dm.Qsql.Close;
      dm.Qsql.SQL.Text:='select CODMANUTENCAO from TMANUT where CODMANUTENCAO = '+Codigo.Text;
      DM.Qsql.Open;
