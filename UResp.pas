...
procedure TfResp.FormActivate(Sender: TObject);
begin
   if sheetLista.Showing then
      Grade.SetFocus;
end;

procedure TfResp.butCancelarClick(Sender: TObject);
begin
...
procedure TfResp.butOkClick(Sender: TObject);
var tm: string[1];
begin
   dm.IniTrans;
   tm:='A';
...
procedure TfResp.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TfResp.GradeDblClick(Sender: TObject);
begin
...
procedure TfResp.GradeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfResp.FormKeyPress(Sender: TObject; var Key: Char);
begin
   if key = #13 then
   begin
      key:=#0;
...
procedure TfResp.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfResp.butIncClick(Sender: TObject);
begin
   sheetLista.TabVisible:=false;
   sheetCad.TabVisible:=true;
   dsCad.DataSet.Append;
...
procedure TfResp.butExcClick(Sender: TObject);
begin
   if dsCad.DataSet.IsEmpty then
      exit;
   if dm.Integridade2('TIORDEM','CODRESP',dm.RespCODRESP.Text) = false then
      exit;
...
procedure TfResp.ButFecharClick(Sender: TObject);
begin
   close;
end;

procedure TfResp.butPesqClick(Sender: TObject);
begin
...
procedure TfResp.GradeEnter(Sender: TObject);
begin
   fResp.KeyPreview:=false;
end;

procedure TfResp.GradeExit(Sender: TObject);
begin
   fResp.KeyPreview:=true;
end;

procedure TfResp.butAltClick(Sender: TObject);
begin
   if dsCad.DataSet.IsEmpty then
      exit;
   dsCad.DataSet.Edit;
...
procedure TfResp.butConfClick(Sender: TObject);
begin
   vgcodigo:=dsCad.DataSet.Fields[0].Text;
   close;
end;

procedure TfResp.CodigoExit(Sender: TObject);
begin
   if Codigo.Modified then
   begin
      dm.Qsql.Close;
      dm.Qsql.SQL.Text:='select CODRESP from TRESP where CODRESP = "'+Codigo.Text+'"';
      DM.Qsql.Open;
...
procedure TfResp.FormCreate(Sender: TObject);
begin
   sheetCad.TabVisible:=false;
