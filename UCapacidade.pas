...
uses UDM, UDMBanco, UUtil, UPesquisa, UAgente, UMarca;
...
procedure TfCapacidade.butMarcaClick(Sender: TObject);
begin
   vgcodigo:='';
   procura(dm.Marca,'%');
   ...
   begin
      ...
      dm.CapacidadeCODMARCA.Text:=vgcodigo;
...
procedure TfCapacidade.pesquisaMarca(Tipo: string);
begin
   ...
   begin
      ...
      if dm.Qsql.RecordCount = 0 then
      begin
         dm.CapacidadeCODMARCA.Clear;
         DescMarca.Clear;
      end;
      if dm.Qsql.RecordCount = 1 then
      begin
         dm.CapacidadeCODMARCA.Text:=dm.Qsql.fieldbyname('CODMARCA').Text;
         DESCMARCA.Text:=dm.Qsql.fieldbyname('DESCMARCA').Text;
      end;
      if dm.Qsql.RecordCount > 1 then
      begin
...
procedure TfCapacidade.CodMarcaExit(Sender: TObject);
begin
  if CodMarca.Modified then
  begin
...
procedure TfCapacidade.DescMarcaExit(Sender: TObject);
begin
...
begin
   ...
   if CodMarca.Text <> '' then
   begin
      ComandoQuery(dm.Qsql,'select DESCMARCA from TMARCA where CODMARCA = '+CodMarca.Text);
      DescMarca.Text := dm.Qsql.FieldByName('DESCMARCA').AsString;
      dm.Qsql.Close;
   end;
end;
