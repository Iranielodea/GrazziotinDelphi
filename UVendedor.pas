...
procedure TfVendedor.butCancelarClick(Sender: TObject);
begin
...
procedure TfVendedor.butOkClick(Sender: TObject);
var tm: string[1];
begin
   dm.IniTrans;
   tm:='A';
...
procedure TfVendedor.GradeDblClick(Sender: TObject);
begin
...
procedure TfVendedor.GradeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfVendedor.butIncClick(Sender: TObject);
begin
   sheetLista.TabVisible:=false;
   sheetCad.TabVisible:=true;
   dsCad.DataSet.Append;
...
procedure TfVendedor.butExcClick(Sender: TObject);
begin
   if dsCad.DataSet.IsEmpty then
      exit;
   if dm.Integridade('TPESSOA','CODVENDEDOR',dm.VendedorCODVENDEDOR.Text) = false then
      exit;
   if dm.Integridade('TPROPOS','CODVENDEDOR',dm.VendedorCODVENDEDOR.Text) = false then
      exit;
...
procedure TfVendedor.ButFecharClick(Sender: TObject);
begin
   close;
end;

procedure TfVendedor.butPesqClick(Sender: TObject);
begin
...
procedure TfVendedor.GradeEnter(Sender: TObject);
begin
   fVendedor.KeyPreview:=false;
end;

procedure TfVendedor.GradeExit(Sender: TObject);
begin
   fVendedor.KeyPreview:=true;
end;

procedure TfVendedor.butAltClick(Sender: TObject);
begin
   if dsCad.DataSet.IsEmpty then
      exit;
   dsCad.DataSet.Edit;
...
procedure TfVendedor.butConfClick(Sender: TObject);
begin
