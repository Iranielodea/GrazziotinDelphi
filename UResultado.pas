...
procedure TfResultado.FormActivate(Sender: TObject);
begin
   if sheetLista.Showing then
      Grade.SetFocus;
end;

procedure TfResultado.butCancelarClick(Sender: TObject);
begin
...
procedure TfResultado.butOkClick(Sender: TObject);
var tm: string[1];
begin
   dm.IniTrans;
   tm:='A';
...
procedure TfResultado.GradeDblClick(Sender: TObject);
begin
...
procedure TfResultado.GradeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfResultado.FormKeyPress(Sender: TObject; var Key: Char);
begin
   if key = #13 then
   begin
      key:=#0;
...
procedure TfResultado.butIncClick(Sender: TObject);
begin
   sheetLista.TabVisible:=false;
   sheetCad.TabVisible:=true;
   dsCad.DataSet.Append;
...
procedure TfResultado.butExcClick(Sender: TObject);
begin
   if dsCad.DataSet.IsEmpty then
      exit;
   if dm.Integridade('TIORDEM','CODRESULTADO',dm.ResultadoCODRESULTADO.Text) = false then
      exit;
...
procedure TfResultado.ButFecharClick(Sender: TObject);
begin
   close;
end;

procedure TfResultado.butPesqClick(Sender: TObject);
begin
...
procedure TfResultado.GradeEnter(Sender: TObject);
begin
   fResultado.KeyPreview:=false;
end;

procedure TfResultado.GradeExit(Sender: TObject);
begin
   fResultado.KeyPreview:=true;
end;

procedure TfResultado.butAltClick(Sender: TObject);
begin
   if dsCad.DataSet.IsEmpty then
      exit;
   dsCad.DataSet.Edit;
...
procedure TfResultado.butConfClick(Sender: TObject);
begin
