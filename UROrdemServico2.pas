...
var
  fROrdemServico2: TfROrdemServico2;
...
procedure TfROrdemServico2.OrdemCalcFields(DataSet: TDataSet);
begin
   Qemp.Open;
...
procedure TfROrdemServico2.RelatorioPreviewFormCreate(Sender: TObject);
begin
   TPPReport(Sender).PreviewForm.WindowState:=wsMaximized;
   TPPViewer(TPPReport(Sender).PreviewForm.Viewer).ZoomPercentage:=100;
end;

procedure TfROrdemServico2.ItensCalcFields(DataSet: TDataSet);
var vpeca: string[10];
   aux: integer;
begin
   vpeca:='';
...
procedure TfROrdemServico2.Impressao(pNumOrdem: integer);
begin
   Ordem.close;
...
procedure TfROrdemServico2.ImprimeNivel;
begin
  QNivel.Close;
  QNivel.Params[0].AsInteger := OrdemNUMORDEM.AsInteger;
  QNivel.Open;

  if QNivelNIVEL.IsNull then
  begin
    MemoNivel.Visible := False;
    lblNiveis.Visible := False;
  end
  else begin
    while not QNivel.Eof do
    begin
      MemoNivel.Lines.Add(QNivelNIVEL.AsString);
      QNivel.Next;
    end;
  end;
  QNivel.Close;
...
procedure TfROrdemServico2.ImprimePecas;
var
