...
procedure TfMarca.butCancelarClick(Sender: TObject);
begin
...
procedure TfMarca.butOkClick(Sender: TObject);
var tm: string[1];
begin
   dm.IniTrans;
   tm:='A';
...
procedure TfMarca.GradeDblClick(Sender: TObject);
begin
...
procedure TfMarca.GradeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfMarca.FormKeyPress(Sender: TObject; var Key: Char);
begin
   if key = #13 then
   begin
      key:=#0;
...
procedure TfMarca.butIncClick(Sender: TObject);
begin
   sheetLista.TabVisible:=false;
   sheetCad.TabVisible:=true;
   dsCad.DataSet.Append;
...
procedure TfMarca.butExcClick(Sender: TObject);
begin
   if dsCad.DataSet.IsEmpty then
      exit;
   if dm.Integridade('TIORDEM','CODMARCA',dm.MarcaCODMARCA.Text) = false then
      exit;
   if dm.Integridade('TCLIEXT','CODMARCA',dm.MarcaCODMARCA.Text) = false then
      exit;
...
procedure TfMarca.ButFecharClick(Sender: TObject);
begin
   close;
end;

procedure TfMarca.butPesqClick(Sender: TObject);
begin
...
procedure TfMarca.GradeEnter(Sender: TObject);
begin
   fMarca.KeyPreview:=false;
end;

procedure TfMarca.GradeExit(Sender: TObject);
begin
   fMarca.KeyPreview:=true;
end;

procedure TfMarca.butAltClick(Sender: TObject);
begin
   if dsCad.DataSet.IsEmpty then
      exit;
   dsCad.DataSet.Edit;
...
procedure TfMarca.butConfClick(Sender: TObject);
begin
