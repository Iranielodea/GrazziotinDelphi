...
procedure TfLocal.FormActivate(Sender: TObject);
begin
   if sheetLista.Showing then
      Grade.SetFocus;
end;

procedure TfLocal.butCancelarClick(Sender: TObject);
begin
...
procedure TfLocal.butOkClick(Sender: TObject);
var tm: string[1];
begin
   dm.IniTrans;
   tm:='A';
...
procedure TfLocal.GradeDblClick(Sender: TObject);
begin
...
procedure TfLocal.GradeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfLocal.FormKeyPress(Sender: TObject; var Key: Char);
begin
   if key = #13 then
   begin
      key:=#0;
...
procedure TfLocal.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfLocal.butIncClick(Sender: TObject);
begin
   sheetLista.TabVisible:=false;
   sheetCad.TabVisible:=true;
   dsCad.DataSet.Append;
...
procedure TfLocal.butExcClick(Sender: TObject);
begin
   if dsCad.DataSet.IsEmpty then
      exit;
   if dm.Integridade('TIORDEM','CODLOCAL',dm.LocalCODLOCAL.Text) = false then
      exit;
   if dm.Integridade('TCLIEXT','CODLOCAL',dm.LocalCODLOCAL.Text) = false then
      exit;
...
procedure TfLocal.ButFecharClick(Sender: TObject);
begin
   close;
end;

procedure TfLocal.butPesqClick(Sender: TObject);
begin
...
procedure TfLocal.GradeEnter(Sender: TObject);
begin
   fLOCAL.KeyPreview:=false;
end;

procedure TfLocal.GradeExit(Sender: TObject);
begin
   fLOCAL.KeyPreview:=true;
end;

procedure TfLocal.butAltClick(Sender: TObject);
begin
   if dsCad.DataSet.IsEmpty then
      exit;
   dsCad.DataSet.Edit;
...
procedure TfLocal.butConfClick(Sender: TObject);
begin
